...
class Trainer:
    def __init__(self, options):
        ...
        self.base_model = self.opt.base_model

        if self.base_model not in _MODEL_MODULE_MAP:
            raise ValueError(
        ...
        if self.base_model == 'litemono':
            ...
            del self.opt.scales[-1]
        elif self.base_model == 'monodepth2':
    ...
    def process_batch(self, inputs):
        ...
        if self.opt.pose_model_type == "shared":
            ...
            outputs = self.models["depth"](features, inputs['obj_mask', 0, 0].float())
    ...
    def compute_losses(self, inputs, outputs):
        ...
        for scale in self.opt.scales:
            ...
            #weights = F.interpolate(outputs[("w", 0)], [self.opt.height, self.opt.width], mode="bilinear", align_corners=False)
            #mot_mask = inputs[("seg", 0, 0)] >= 24
            ...
            pose_loss += self.opt.disparity_smoothness * smooth_loss / (2 ** scale)
            total_loss += loss
